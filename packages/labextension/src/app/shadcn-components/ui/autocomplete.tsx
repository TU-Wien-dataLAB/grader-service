...
import { ChevronRight, Info, SearchIcon, X } from 'lucide-react';
  ...
  'bg-input text-foreground placeholder:text-muted-foreground [[readonly]]:bg-muted/80 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 dark:bg-input/30 flex max-w-[400px] min-w-0 rounded-xs text-sm transition-colors outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-3 [[readonly]]:cursor-not-allowed',
  {
    variants: {
      size: {
        ...
        default:
          'h-8 px-2 py-1.5 [&~[data-slot=autocomplete-clear]]:right-1.75 [&~[data-slot=autocomplete-trigger]]:right-1.75',
  ...
  return (
    ...
    >
      <AutocompletePrimitive.List
        ...
        className={cn(
          'not-empty:scroll-py-1 in-data-has-overflow-y:mr-3',
          className
        )}
        {...props}
      />
...
const AutocompleteItem = ({
  ...
  return (
    <AutocompletePrimitive.Item
      ...
      className={cn(
        "text-foreground data-highlighted:text-foreground data-highlighted:before:bg-accent relative flex cursor-default items-center gap-1.5 rounded-none focus:ring-2 focus:ring-primary focus:ring-offset-1 px-2 py-1.5 text-sm outline-hidden transition-colors select-none data-disabled:pointer-events-none data-disabled:opacity-50 data-highlighted:relative data-highlighted:z-0 data-highlighted:before:absolute data-highlighted:before:inset-x-0 data-highlighted:before:inset-y-0 data-highlighted:before:z-[-1] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[svg]:shrink-0 [&_svg:not([role=img]):not([class*=text-])]:opacity-60",
        className
      )}
      {...props}
    >
      ...
      >
        <div className="relative flex max-h-full">
          <AutocompletePrimitive.Popup
            ...
            className={cn(
              'text-popover-foreground ring-foreground/10 bg-popover flex max-h-[min(var(--available-height),24rem)] w-(--anchor-width) max-w-80 origin-(--transform-origin) scroll-pt-2 scroll-pb-2 flex-col overscroll-contain rounded-xs shadow-md ring-1 transition-[scale,opacity] has-data-starting-style:scale-98 has-data-starting-style:opacity-0 has-data-[side=none]:scale-100 has-data-[side=none]:transition-none',
              className
            )}
            {...props}
          >
...
const AutocompleteEmpty = ({
  ...
  return (
    <AutocompletePrimitive.Empty
      ...
      className={cn(
        'px-2 py-1.5 text-center text-sm empty:m-0 empty:p-0',
        className
      )}
      {...props}
    >
      <div
        className={
          'flex flex-row bg-chart-1 border border-primary items-center gap-2 p-4 text-center'
        }
      >
        <Info className={'size-6 fill-primary text-white'} />
        {children}
      </div>
    </AutocompletePrimitive.Empty>
  );
...
const AutocompleteClear = ({
  ...
  return (
    <AutocompletePrimitive.Clear
      ...
      className={cn(
        'text-muted-foreground p-2 ring-offset-background focus:ring-ring absolute top-1/2 -translate-y-1/2 cursor-pointer focus:ring-2 focus:ring-offset-2 transition-opacity focus:outline-none disabled:pointer-events-none data-disabled:pointer-events-none',
        className
      )}
      {...props}
    >
...
const AutocompleteTrigger = ({
  ...
  return (
    <AutocompletePrimitive.Trigger
      ...
      className={cn(
        'bg-sidebar-ring p-2 focus:ring-ring ring-offset-background absolute top-1/2 -translate-y-1/2 cursor-pointer focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:pointer-events-none has-[+[data-slot=autocomplete-clear]]:hidden data-disabled:pointer-events-none',
        className
      )}
      {...props}
    >
      <SearchIcon className="size-4 text-primary opacity-70" />
    </AutocompletePrimitive.Trigger>
  );
};
